...
process_station_worker <- function(station_name, data_full) {
  ...
  res <- tryCatch({
    ...
    data.frame(
      ...
      Pred_Max_80 <- round(pmax(pred_mean + 1.2816 * pred_sigma, 0), 2),
      Pred_Max_95 <- round(pmax(pred_mean + 1.96   * pred_sigma, 0), 2)
    ) %>%
      mutate(
        Statut = case_when(
          Pred_Max_95 > SEUIL_ALERTE ~ "Alerte grave",
          Pred_Max_80 > SEUIL_ALERTE ~ "Alerte moyenne",
          Pred_Max    > SEUIL_ALERTE ~ "Alerte légère",
          TRUE                       ~ "Normal"
        )
      )
